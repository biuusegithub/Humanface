...
def validation(example, tokenizer, image_encoder, text_encoder, unet, mapper, mapper_local, vae, device, guidance_scale, token_index=0, seed=42, llambda=1):
    ...
    image_embeddings = [image_features[2][i] for i in range(25)]
    image_embeddings.insert(0, image_features[0])   
...
def main():
    ...
    mapper_local = Mapper(input_dim=1024, output_dim=768)
    ...
    for _name, _module in unet.named_modules():
        if _module.__class__.__name__ == "CrossAttention":
            ...
            shape = _module.to_q.weight.shape
            to_q_global = nn.Linear(shape[1], shape[0], bias=False)
            to_q_global.weight.data = _module.to_q.weight.data.clone().to("cuda")
            mapper.add_module(f'{_name.replace(".", "_")}_to_q', to_q_global)            
            ...
            if args.global_mapper_path is None:
                _module.add_module('to_q_global', to_q_global)
    ...
    if args.global_mapper_path is not None:
        ...
        for _name, _module in unet.named_modules():
            if _module.__class__.__name__ == "CrossAttention":
                if 'attn1' in _name: continue
                _module.add_module('to_q_global', getattr(mapper, f'{_name.replace(".", "_")}_to_q'))
    ...
    optimizer = torch.optim.AdamW(
        itertools.chain(mapper.parameters(), mapper_local.parameters()),
    ...
    noise_scheduler = DDPMScheduler.from_config(args.ddim_path)
    # noise_scheduler = DPMSolverMultistepScheduler.from_config(args.ddim_path)
